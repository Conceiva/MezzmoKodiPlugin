...
def addTrailers(dbsync, mtitle, trailers, prflocaltr, myear, mpremiered):  #  Add movie trailers to Syncdb
                    ...
                    and localcount == 0):
                        if match == 1:
                            ...
                            try:
                                ...
                                + ' ' + str(e), xbmc.LOGDEBUG)
                        try:
                            dbsync.execute('INSERT into mTrailers (trTitle, trUrl, trID, trPlay, trVar1, trYear, trPremiered)  \
                            values (?, ?, ?, ?, ?, ?, ?)', (mtitle, trailer, str(a), "0", orgtrailer, int(myear), mpremiered,))
                        except:
                            dbsync.execute('INSERT into mTrailers (trTitle, trUrl, trID, trPlay, trVar1, trYear, trPremiered) \
                            values (?, ?, ?, ?, ?, ?, ?)', (mtitle, trailer, str(a), "0", "Unable to decode local trailer",   \
                            int(myear), mpremiered,))
                        a += 1            
            dbsync.commit()
...
def mgenlogUpdate(mgenlog):                              #  Add Mezzmo general logs to DB
    ...
    try:
        ...
        try:                                             #  Handle utf-8 encoding in logs
            mgenlogd = mgenlog.decode('utf-8')
        except:
            mgenlogd = mgenlog
        currmsDate = datetime.now().strftime('%Y-%m-%d')
        currmsTime = datetime.now().strftime('%H:%M:%S:%f')
        mgfile.execute('INSERT into mgenLog(mgDate, mgTime, mgGenDat) values (?, ?, ?)',                \
        (currmsDate, currmsTime, mgenlogd))
    ...
    if movieId != 0:
        ...
        for actor in actorlist:     
            ...
            if actortuple:                   #  Insert actor to movie link in actor link table
                ...
                if media_type == 'episode' and mnativeact == 'true':
                    db.execute('INSERT OR REPLACE into ACTOR_LINK (actor_id, media_id, media_type, cast_order) values \
                    (?, ?, ?, ?)', (actornumb, mshowId, 'tvshow', ordernum,)) 
...
def insertSets(movienumb, db, setname, knative, murl, micon):
    ...
    db.execute('UPDATE movie SET idSet=? WHERE idMovie=?', (None, movienumb,))      # Clear movieset info in movie table
    ...
    if sagtuple:                         #  Insert set to movie table
        ...
        #xbmc.log('The current set number is: ' + str(setnumb) + "  " + str(msetname), xbmc.LOGNOTICE)
...
def insertTags(movienumb, db, media_type, keywords):
    ...
    xbmc.log('Mezzmo taglist is: ' + str(taglist).encode('utf-8'), xbmc.LOGDEBUG)    # tags insertion debugging
    for tag in taglist:     
        xbmc.log('Mezzmo current tag is: ' + str(tag.encode('utf-8')), xbmc.LOGDEBUG)
        mtag = tag.strip()
...
def insertDirectors(movienumb, db, mtype, directors, imageSearchUrl, kdirector):
    ...
    try:
        ...
        if directors == None or len(directors) == 0:
            return
        directorlist = directors.encode('utf-8', 'ignore').replace(', Jr.' , ' Jr.').replace(', Sr.' , ' Sr.').split(', ')  # Convert to list
        ...
        for director in directorlist:
            ...
            curg = db.execute('SELECT actor_id FROM actor WHERE name=?',(mdirector.decode('utf-8'),))   
            ...
            if not directortuple:                   #  If director not in actor table insert and fetch new actor ID
                db.execute('INSERT into actor (name, art_urls) values (?, ?)', (mdirector.decode('utf-8'), searchUrl,))
                cur = db.execute('SELECT actor_id FROM actor WHERE name=?',(mdirector.decode('utf-8'),))   
            ...
            if directortuple:                       #  Insert director to movie link in director link table
                ...
                #xbmc.log('The current director number is: ' + str(directornumb) + "  " + str(movienumb), xbmc.LOGNOTICE)

    except Exception as e:
        printexception()
        msynclog ='Mezzmo problem inserting directors for: ' + str(directors).encode('utf-8') + ' ' + str(movienumb)
        xbmc.log(msynclog, xbmc.LOGNOTICE)
...
def insertWriters(movienumb, db, mtype, writers, imageSearchUrl, kdirector):
    ...
    try:
        ...
        if writers == None or len(writers) == 0:
            return
        writerlist = writers.encode('utf-8', 'ignore').replace(', Jr.' , ' Jr.').replace(', Sr.' , ' Sr.').split(', ')  # Convert to list
        ...
        for writer in writerlist:
            ...
            curg = db.execute('SELECT actor_id FROM actor WHERE name=?',(mwriter.decode('utf-8'),))   
            ...
            if not writertuple:                   #  If writer not in actor table insert and fetch new actor ID
                db.execute('INSERT into actor (name, art_urls) values (?, ?)', (mwriter.decode('utf-8'), searchUrl,))
                cur = db.execute('SELECT actor_id FROM actor WHERE name=?',(mwriter.decode('utf-8'),))   
            ...
            if writertuple:                       #  Insert writer to movie link in writer link table
                ...
                #xbmc.log('The current writer number is: ' + str(writernumb) + "  " + str(movienumb), xbmc.LOGNOTICE)
...
def insertStudios(movienumb, db, media_type, studios, knative):
    ...
    try:
        ...
        if studios == None or len(studios) == 0:
            return    
        studiolist = studios.encode('utf-8', 'ignore').split(',')                        # Convert studios to list
        ...
        for studio in studiolist:     
            ...
            curs = db.execute('SELECT studio_id FROM studio WHERE name=?',(mstudio.decode('utf-8'),))   
            ...
            if not studiotuple:                   #  If studio not in studio table insert and fetch new studio ID
                db.execute('INSERT into STUDIO (name) values (?)', (mstudio.decode('utf-8'),))
                cur = db.execute('SELECT studio_id FROM studio WHERE name=?',(mstudio.decode('utf-8'),))   
            ...
            if studiotuple:                       #  Insert studio to movie link in studio link table
                ...
                #xbmc.log('The current studio number is: ' + str(studionumb) + "  " + str(movienumb), xbmc.LOGNOTICE)
