...
def displayPerfStats():
    ...
    if vdate < 0:                                             # User cancel
        ...
        for rlist in pstatlists:
            ...
            plists.append(x)                                  # Convert rows to list for dialog box
    ...
    if pstatslist and vdate >= 0:                            # Check for records in perfdb and not cancel
        ...
        xbmc.log('Mezzmo no performance stats found in database. ', xbmc.LOGINFO)       
    pdfile.close()


def displayDupeLogs():
    if int(get_installedversion()) < 18:                          # Textviewer function added in Kodi 18
        dialog_text = "Mezzmo Dupe Logs are only available in Kodi version 18 and higher."
        xbmcgui.Dialog().ok("Mezzmo Dupe Logs", dialog_text)
        return       
    ...
    textval1 = "{:^25}".format("Date") + "{:^21}".format("Record #") + "{:>24}".format("Playlist") +  "\n" 
    headval = "Mezzmo Dupe Logs"
    dialog = xbmcgui.Dialog()

    if dllogs:
        for a in range(len(dllogs)):                              # Display logs   
            dldate = dllogs[a][0]
            recnumb = dllogs[a][1]
            ctitle = "                " + dllogs[a][3]
        ...
        dialog.textviewer(headval, textval1)
        xbmc.log('Mezzmo no dupe logs found in database. ', xbmc.LOGINFO)                
    dlfile.close()
    ...
    if syncSelect == "All Sync Logs":
        ...
        if mstatdates:        
            for rdate in mstatdates:
                x = str(rdate).replace("('","").replace("',)","")
                msdates.append(x)                                 # Convert rows to list for dialog box
            mdate = msdialog.select('Select Sync Logs Date', msdates)
            cursync = dsfile.execute('SELECT * FROM msyncLog WHERE msDate=? ORDER BY msTime DESC', (msdates[mdate],))
            headval = 'Mezzmo Sync Logs for:  ' + msdates[mdate][5:] + "-" + \
            msdates[mdate][:4]
        else:                                                     # No sync logs found for date selected
            textval1 = "No Mezzmo sync logs found."
